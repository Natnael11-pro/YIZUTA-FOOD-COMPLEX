...
const AdminPanel = () => {
  const { userRole, userId } = useAuth() // Get userId from auth context
  const canModifyUsers = userRole === 'admin'
  ...
  // ✅ UPDATED: Prevent deleting own account
  const handleDeleteUser = async (targetUserId: string) => {
    ...
    try {
      // Use the RPC function to delete from both Auth and Profiles
      const { error } = await supabase
        .rpc('delete_user_from_auth', { user_id: targetUserId })
                  ...
                  <tr key={user.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-3">
                        ...
                        {/* ✅ Only show delete if it's not the current user */}
                        {user.id !== userId ? (
                          <button 
